...
echo "🚀 Regenerating all JSON files with 100% accurate original data..."
...
cat > data/installations/Inland_Wonderland.json << 'EOF'
{
  ...
  "longDescription": "Dedicated software developed in Godot game engine / Accelerometer data collection with an Arduino / Interactive 2d animation loop using sensor data.",
...
cat > data/performances/Save_the_Last_Trance_for_Me.json << 'EOF'
{
  ...
  "longDescription": "Dedicated video player in MaxMSP/Jitter and Ableton LIVE / Editable cues for playback triggers & smooth transitions / Editable video effect chains using pixel shaders inside MaxForLive modules / Kinect tracking a lamp torch and mapping of its position to the screens.",
...
echo "✅ All 27 JSON files + manifest.json have been recreated with accurate original content."
echo "You can now refresh http://localhost:8000"
